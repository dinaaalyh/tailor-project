...
<body
    ...
    <main class="relative h-full max-h-screen transition-all duration-200 ease-in-out xl:ml-68 rounded-xl">
        ...
        <!-- Modal Approve: CENTERED -->
        <!-- FLOW MODAL (dinamis: Process / Complete) -->
        <div id="flow-modal"
            class="fixed inset-0 z-[9999] hidden flex items-center justify-center p-4 bg-yellow-100/40">
            <div class="w-[90%] max-w-md rounded-xl bg-white shadow-xl border border-slate-200">
                <div class="px-5 py-4 border-b">
                    <h3 id="flow-title" class="text-base font-semibold text-slate-700">Setujui Pesanan</h3>
                    <p class="mt-1 text-xs text-slate-500">
                        Pesanan akan diproses sesuai tindakan yang dipilih.
                    </p>
                </div>
                <div class="px-5 py-4">
                    <p id="flow-body" class="text-sm text-slate-600">
                        Apakah kamu yakin?
                    </p>
                </div>
                <div class="px-5 py-4 border-t flex items-center justify-end gap-2">
                    <button type="button" onclick="closeFlowModal()"
                        class="h-9 px-3 rounded-md border border-slate-300 text-slate-700 hover:bg-slate-50 text-sm">
                        Batal
                    </button>
                    <button id="flow-primary" type="button" onclick="confirmFlow()"
                        class="h-9 px-4 rounded-md text-white text-sm" style="background:#f59e0b">
                        Lanjut
                    </button>
                </div>
            </div>
        ...
        <!-- Modal Cancel: CENTERED -->
        <div id="cancel-modal" class="fixed inset-0 z-[9999] hidden flex items-center justify-center p-4">
            <div class="w-[90%] max-w-md rounded-xl bg-white shadow-xl border border-slate-200">
                <div class="px-5 py-4 border-b">
                    <h3 class="text-base font-semibold text-slate-700">Selesaikan Pesanan</h3>
                    <p class="mt-1 text-xs text-slate-500">Menandai pesanan ini sebagai <b>Selesai</b>.</p>
                </div>
                <div class="px-5 py-4">
                    <p class="text-sm text-slate-600">
                        Apakah kamu yakin ingin menandai pesanan ini <span
                            class="font-semibold text-emerald-600">Selesai</span>?
                    </p>
                </div>
                <div class="px-5 py-4 border-t flex items-center justify-end gap-2">
                    <button type="button" onclick="closeCancelModal()"
                        class="h-9 px-3 rounded-md border border-slate-300 text-slate-700 hover:bg-slate-50 text-sm">Batal</button>
                    <button type="button" onclick="confirmCancel()" class="h-9 px-4 rounded-md text-white text-sm"
                        style="background:#16a34a">Selesaikan</button>
                </div>
            </div>
        </div>
                ...
                <div class="p-6 border-b">
                    <div class="grid grid-cols-1 lg:grid-cols-12 items-center gap-4">
                        <div class="lg:col-span-7">
                            <h5 class="text-slate-700 font-semibold">Data Order Pesanan</h5>
                            <p class="mt-1 text-xs text-slate-400">Daftar Order Pesanan</p>
                        </div>

                        <!-- Search kanan & sejajar -->
                        {{-- <div class="lg:col-span-5 flex items-center justify-end">
                            <label class="relative w-full sm:w-64">
                                ...
                                <input id="search-stok" type="text" placeholder="Cari…"
                                    class="h-10 w-full pl-8 pr-3 rounded-lg border border-slate-300 text-sm placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none min-w-[220px]">
                            </label>
                        </div> --}}
                    </div>
                </div>
                        ...
                        <tbody class="divide-y">
                            ...
                            <tr id="order-1" class="text-sm">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span
                                        class="status-badge inline-flex items-center px-3 py-1 text-xs font-semibold rounded-md text-white"
                                        data-status="new" style="background:#2563eb">
                                        New order
                                    </span>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <!-- PROCESS -->
                                        <!-- PROCESS -->
                                        <button type="button" title="Process" onclick="openFlowModal(this)"
                                            class="rounded-md text-white ring-1 ring-amber-700/30 hover:brightness-110"
                                            style="width:36px;height:36px;display:grid;place-items:center;background-color:#f59e0b;">
                                            <!-- ikon “proses” -->
                                        ...
                                        <!-- COMPLETE -->
                                        <button type="button" title="Complete" onclick="openFlowModal(this)"
                                            class="rounded-md text-white ring-1 ring-green-700/30 hover:brightness-110"
                                            style="width:36px;height:36px;display:grid;place-items:center;background-color:#16a34a;">
                                            <!-- check -->
        ...
        <!-- Script -->
        <script>
            // === STATE untuk Flow (Process -> Done) ===
            let __activeRowId = null;
            let __nextStatus = null;
            ...
            function readStatus(row) {
                ...
                if (!b) return 'new';
                let s = (b.dataset.status || '').toLowerCase();
                if (!s) {
                    const t = (b.textContent || '').toLowerCase();
                    if (t.includes('selesai')) s = 'done';
                    else if (t.includes('proses')) s = 'processing';
                    else s = 'new';
                }
                return s;
            }
            ...
            function applyStatus(row, state) {
                const b = row.querySelector('.status-badge');
                if (!b) return;
                const st = STATUS[state] || STATUS.new;
            ...
            // === MODAL FLOW (dipanggil dari tombol "Process" & "Complete") ===
            function openFlowModal(btn) {
                const tr = btn.closest('tr');
                if (!tr || !tr.id) return;
                const current = readStatus(tr);
                if (current === 'done') return; // sudah selesai

                __activeRowId = tr.id;
                __nextStatus = (current === 'new') ? 'processing' : 'done';

                const modal = document.getElementById('flow-modal');
                const title = document.getElementById('flow-title');
                const body = document.getElementById('flow-body');
                const prim = document.getElementById('flow-primary');

                if (__nextStatus === 'processing') {
                    title.textContent = 'Setujui Pesanan';
                    body.innerHTML =
                        'Apakah kamu yakin ingin <span class="font-semibold text-amber-700">mengubah status ke Diproses</span>?';
                    prim.textContent = 'Setujui';
                    prim.style.background = '#f59e0b';
                } else {
                    title.textContent = 'Selesaikan Pesanan';
                    body.innerHTML =
                        'Apakah kamu yakin ingin <span class="font-semibold text-emerald-700">menandai pesanan ini Selesai</span>?';
                    prim.textContent = 'Selesaikan';
                    prim.style.background = '#16a34a';
                }
            ...
            function confirmFlow() {
                if (!__activeRowId || !__nextStatus) return;
                const row = document.getElementById(__activeRowId);
                if (!row) return;

                applyStatus(row, __nextStatus);

                if (__nextStatus === 'done') {
                    // disable tombol setelah selesai
                    const proc = row.querySelector('button[title="Process"]');
                    const comp = row.querySelector('button[title="Complete"]');
                    [proc, comp].forEach(b => {
                        if (b) {
                            b.disabled = true;
                            b.style.opacity = 0.6;
            ...
            // Tutup modal flow via overlay & ESC
